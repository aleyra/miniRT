...
/*   Updated: 2021/03/17 11:21:21 by lburnet          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	on_circle(t_vec3 center, float diam, t_vec3 p)
{
	...
	eq = pow(p.x - center.x, 2) + pow(p.y - center.y, 2) - pow(diam * 0.5, 2);
...
int	on_cylinder(t_obj *cy, t_vec3 p)
{
	...
	r1 = angle_one_polaris(cy->norm);
	r2 = amgle_two_polaris(cy->norm);
